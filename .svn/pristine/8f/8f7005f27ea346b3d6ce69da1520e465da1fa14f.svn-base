...
long_t compute_alu(alu_t op, long_t argA, long_t argB)
{
    ...
    switch (op) {
        case A_ADD: val = argB + argA; break;
        case A_SUB: val = argB - argA; break;
        case A_AND: val = argB & argA; break;
        case A_XOR: val = argB ^ argA; break;
        default: break;
    }
...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    bool_t is_negA = (argA >> 63) & 0x1;
    bool_t is_negB = (argB >> 63) & 0x1;
    bool_t is_negV = (val >> 63) & 0x1;

    ovf = (((op == A_ADD) && (!(is_negA ^ is_negB) && (is_negA ^ is_negV))) || 
            ((op == A_SUB) && ((is_negA ^ is_negB) && (is_negB ^ is_negV))));
...
bool_t cond_doit(cc_t cc, cond_t cond) 
{
    ...
    switch (cond) {
        ...
        case C_LE: doit = ZF | (SF ^ OF); break;
        case C_L: doit = SF ^ OF; break;
        case C_E: doit = ZF; break;
        case C_NE: doit = !ZF; break;
        case C_GE: doit = !(SF ^ OF); break;
        case C_G: doit = !(ZF | (SF ^ OF)); break;
    }
...
stat_t nexti(y64sim_t *sim)
{
    ...
    regid_t regA = GET_REGA(regAB);
    regid_t regB = GET_REGB(regAB);
    ...
    switch (icode) {
        ...
        case I_CALL: { /* 8:x imm */
            long_t valRSP = get_reg_val(sim->r, REG_RSP) - 8;
        ...
        case I_POPQ: { /* B:0 regA:F */
            long_t valRSP = get_reg_val(sim->r, REG_RSP);
            long_t valM = 0;
            if (!get_long_val(sim->m, valRSP, &valM)) {
                err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
                return STAT_ADR;
            }
            set_reg_val(sim->r, REG_RSP, valRSP + 8);
