...
void write_long(bin_t *b, long *data, int pos) { //pos = 0 or 1 or 2
    for (int i = 0; i < 8; i++) {
        b->codes[i + pos] = (*data >> (8 * i)) & 0xFF;
    }
}
...
{
    symbol_t *cur_symbol = symtab->next; // pretend symtab has the head node.
    while (cur_symbol) {
        if (strcmp(cur_symbol->name, name) == 0) break;
...
parse_t parse_symbol(char **ptr, char **name)
{
    ...
    /* skip the blank and check */
    SKIP_BLANK(cur_ptr);
    if (!IS_LETTER(cur_ptr)) return PARSE_ERR;

    /* allocate name and copy to it */
    char *sym_name = (char *)malloc(64 * sizeof(char));
    char *cur_p = sym_name;
...
parse_t parse_digit(char **ptr, long *value)
{
    ...
    /* calculate the digit, (NOTE: see strtoll())->useless!!! */
    const char *begin = *ptr;
    *value = strtoull(begin, ptr, 0);
...
parse_t parse_imm(char **ptr, char **name, long *value)
{
    ...
    /* skip the blank and check */
    SKIP_BLANK(cur_ptr);
    if (!IS_IMM(cur_ptr) && !IS_LETTER(cur_ptr)) return PARSE_ERR;
    ...
    /* if IS_LETTER, then parse the symbol */
        ...
        if (parse_symbol(&cur_ptr, name) == PARSE_ERR) return PARSE_ERR;
        *ptr = cur_ptr;
        return PARSE_SYMBOL;
    }
...
parse_t parse_mem(char **ptr, long *value, regid_t *regid)
{
    ...
    cur_ptr++;
    if (parse_reg(&cur_ptr, regid) == PARSE_ERR) 
        return PARSE_ERR;
    if (*cur_ptr != ')') {
        err_print("Invalid MEM");
        return PARSE_ERR;
...
parse_t parse_data(char **ptr, char **name, long *value)
{
    /* skip the blank and check */
    SKIP_BLANK(*ptr);
    if (!IS_DIGIT(*ptr) && !IS_LETTER(*ptr)) return PARSE_ERR;
...
*           call SUM  #invoke SUM function */
    ...
    if (inst) {
        ...
        switch (icode) {
            ...
            case I_ALU: {
                regid_t rA, rB;
                if (parse_reg(&ptr, &rA) == PARSE_ERR) return TYPE_ERR;
                if (parse_delim(&ptr, ',') == PARSE_ERR) return TYPE_ERR;
                if (parse_reg(&ptr, &rB) == PARSE_ERR) return TYPE_ERR;
                line->y64bin.codes[1] = HPACK(rA, rB);
                break;
            }
            case I_IRMOVQ: {
                long imm; regid_t rB; char *name;
                switch (parse_imm(&ptr, &name, &imm)) {
                    case PARSE_SYMBOL: {
                        add_reloc(name, &(line->y64bin));
                        break;
                    }
                    case PARSE_DIGIT: {
                        write_long(&(line->y64bin), &imm, 2);
                        break;
                    }
            ...
            case I_RMMOVQ: {
                long *imm = (long *)malloc(sizeof(long)); 
                regid_t rA, rB;
                if (parse_reg(&ptr, &rA) == PARSE_ERR) return TYPE_ERR;
                if (parse_delim(&ptr, ',') == PARSE_ERR) return TYPE_ERR;
                if (parse_mem(&ptr, imm, &rB) == PARSE_ERR) return TYPE_ERR;
                line->y64bin.codes[1] = HPACK(rA, rB);
                write_long(&(line->y64bin), imm, 2);
                break;
            }
            ...
            case I_DIRECTIVE: {
                switch (ifun) {
                    case D_DATA: {
                        char *name; long *value = (long *)malloc(sizeof(long));
                        switch (parse_data(&ptr, &name, value)) {
                            case PARSE_SYMBOL: {
                                add_reloc(name, &(line->y64bin));
                                break;
                            }
                            case PARSE_DIGIT: {
                                write_long(&(line->y64bin), value, 0);
                                break;
                            }
                            default: return TYPE_ERR;
                        }
                    ...
                    case D_ALIGN: {
                        ...
                        if (parse_digit(&ptr, value) == PARSE_ERR) return TYPE_ERR;
    ...
    /* test the rest token */
    SKIP_BLANK(ptr);
    if (!IS_END(ptr) && !IS_COMMENT(ptr)) line->type = TYPE_ERR;
...
int relocate(void)
{
    ...
    while (rtmp) {
        ...
        itype_t tmp_type = HIGH(rtmp->y64bin->codes[0]);
        dtv_t tmp_fun = LOW(rtmp->y64bin->codes[0]);
        int offset = 0;
        switch (tmp_type) {
            case I_IRMOVQ: offset = 2; break;
            case I_JMP:
            case I_CALL: offset = 1; break;
            case I_DIRECTIVE: {
                if (tmp_fun == D_DATA) {
                    offset = 0; break;
                } else return -1;
        ...
        write_long(rtmp->y64bin, &(stmp->addr), offset);
...
int binfile(FILE *out)
{
    ...
    line_t *cur_line = line_head->next;

    /* binary write y64 code to output file (NOTE: see fwrite()) */
    while(cur_line){
        if(cur_line->type == TYPE_INS){
            if(fseek(out, cur_line->y64bin.addr, SEEK_SET) != 0)
                return -1;
            fwrite(cur_line->y64bin.codes, 1, cur_line->y64bin.bytes, out);
        }
